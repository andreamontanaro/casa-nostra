...
- **La barra rapida si legge da sola.** Nell'unico campo si scrive una riga e basta: nome, quantità e tipo di prodotto li ricava Gemini (`readQuickItemInput` in `lib/shopping/service.ts`, stesse definizioni di categoria del tool dell'assistente), quindi «x2 mele» diventa `mele` con quantità `2`. Il toast dice come è stata interpretata. Se il modello manca, tarda o sbaglia, si ripiega sulla riga così com'è, senza quantità e in `cibo` — l'IA qui è un comfort, non una dipendenza: non far mai fallire un'aggiunta perché l'interpretazione non è arrivata.
