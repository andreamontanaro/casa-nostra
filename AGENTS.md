...
- **La barra rapida categorizza da sola.** Nell'unico campo si scrive il nome e basta: la categoria la propone Gemini (`suggestShoppingCategory` in `lib/shopping/service.ts`, stesse definizioni del tool dell'assistente) e il toast dice dov'è finito il prodotto. Se il modello manca, tarda o sbaglia, si ripiega su `cibo` e l'articolo entra lo stesso — l'IA qui è un comfort, non una dipendenza: non far mai fallire un'aggiunta perché la categoria non è arrivata.
