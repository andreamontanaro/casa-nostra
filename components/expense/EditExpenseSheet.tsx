...
export function EditExpenseSheet({ expense, profiles, currentUserId, attachmentCount, returnHref }: {
  expense: Tables<'expenses'>; profiles: Tables<'profiles'>[]; currentUserId: string; attachmentCount: number; returnHref?: string
}) {
  ...
  return <div className="px-4 py-5">
    ...
    <Sheet open={open} onOpenChange={setOpen} title="Modifica spesa" size="full">
      <EditExpenseForm expense={expense} profiles={profiles} currentUserId={currentUserId} attachmentCount={attachmentCount}
        returnHref={returnHref} onSaved={() => setOpen(false)} />
    </Sheet>
  </div>
}
