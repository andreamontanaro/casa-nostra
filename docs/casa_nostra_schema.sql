...
-- ============================================================
-- 10. MODULO "LE MIE AUTO" (gestione personale spese auto)
-- ------------------------------------------------------------
-- Modulo indipendente dalle spese condivise. Le auto sono PRIVATE
-- per utente: RLS owner-scoped (owner_id = auth.uid()), diversamente
-- dal resto dell'app dove i due utenti vedono tutto.
-- Applicato via migration add_cars_module; replicato qui per completezza.
-- ============================================================

-- Enum tipo carburante.
CREATE TYPE public.fuel_type AS ENUM (
  'benzina', 'diesel', 'gpl', 'metano', 'elettrico', 'ibrido'
);

-- Auto personali. photo_path: chiave nel bucket privato car-photos.
...
-- Rifornimenti. liters/price_per_liter/total_cost: il form ne fa inserire due,
-- il terzo si calcola; tutti e tre sono persistiti.
...
CREATE INDEX idx_fuel_entries_car_date ON public.fuel_entries(car_id, entry_date DESC);
CREATE INDEX idx_fuel_entries_car_odo  ON public.fuel_entries(car_id, odometer_km);

-- Letture contachilometri registrate senza un rifornimento (grafici di percorrenza).
...
CREATE INDEX idx_odometer_readings_car_date ON public.odometer_readings(car_id, reading_date DESC);

-- Trigger updated_at (riusa set_updated_at).
...
CREATE TRIGGER trg_fuel_entries_updated_at
  ...
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
CREATE TRIGGER trg_odometer_readings_updated_at
...
CREATE POLICY "fuel_entries_owner_all" ON public.fuel_entries FOR ALL TO authenticated
...
CREATE POLICY "odometer_readings_owner_all" ON public.odometer_readings FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.cars c WHERE c.id = car_id AND c.owner_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM public.cars c WHERE c.id = car_id AND c.owner_id = auth.uid()));

-- Storage: bucket privato car-photos, owner-scoped per cartella.
-- Path: {owner_id}/{car_id}/{uuid}.{ext}
INSERT INTO storage.buckets (id, name, public)
VALUES ('car-photos', 'car-photos', false)
ON CONFLICT (id) DO NOTHING;

CREATE POLICY "cp_select" ON storage.objects FOR SELECT TO authenticated
  USING (bucket_id = 'car-photos' AND (storage.foldername(name))[1] = auth.uid()::text);
CREATE POLICY "cp_insert" ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'car-photos' AND (storage.foldername(name))[1] = auth.uid()::text);
CREATE POLICY "cp_update" ON storage.objects FOR UPDATE TO authenticated
  USING (bucket_id = 'car-photos' AND (storage.foldername(name))[1] = auth.uid()::text)
  WITH CHECK (bucket_id = 'car-photos' AND (storage.foldername(name))[1] = auth.uid()::text);
CREATE POLICY "cp_delete" ON storage.objects FOR DELETE TO authenticated
  USING (bucket_id = 'car-photos' AND (storage.foldername(name))[1] = auth.uid()::text);
