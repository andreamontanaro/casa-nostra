...
* `addQuickItemAction(name)`: Aggiunta dalla barra rapida della lista, dove l'unico campo è il nome. Chiede la categoria a `suggestShoppingCategory` (→ `lib/shopping/service.ts`, dettagli in [07. Integrazioni](07-integrations.md)) e poi delega a `addItemAction` con urgenza `media`. Se il modello non è configurato, è lento o sbaglia, la categoria ripiega su `cibo` e l'articolo entra lo stesso: la barra rapida non può dipendere dall'IA. La categoria scelta torna al client per il toast, così chi aggiunge vede dov'è finito il prodotto.
