...
* `updateExpense(id, _prev, formData)`: Valida e aggiorna la spesa identificata dall'id. Invalida la cache delle rotte interessate e restituisce `{ ok: true }`: si resta sul dettaglio, che la risposta porta già aggiornato, e il form chiude la sheet con il suo toast → `app/actions/expenses.ts`.
* `deleteExpense(id, returnTo?)`: Recupera i percorsi di tutti gli allegati collegati alla spesa, li rimuove fisicamente dal bucket Storage `expense-attachments` e infine elimina la spesa (con cascade SQL sui metadati dell'allegato). Reindirizza alla schermata di partenza (`/` o lo storico con i suoi filtri; solo percorsi interni noti) con `?ok=expense-deleted`. Reindirizza allo storico → `app/actions/expenses.ts#L149`.

### Conguagli (`settlement.ts`)
* `registerSettlement(notes?, expenseIds?)`: Invoca la funzione RPC di database `register_settlement` passando note opzionali e un array opzionale di ID spese. La nota arriva dal campo facoltativo della conferma: viene ripulita dagli spazi, una nota vuota diventa `NULL` e oltre `SETTLEMENT_NOTES_MAX` (200 caratteri, `lib/balance.ts`) l'action risponde con un errore. Invalida la cache delle pagine di riepilogo, accoda la notifica Telegram del conguaglio (con la nota, se c'è) e reindirizza alla home con un parametro di successo → `app/actions/settlement.ts`.
...
* `addQuickItemAction(text)`: Aggiunta dalla barra rapida della lista, dove si scrive una riga sola. Passa il testo a `readQuickItemInput` (→ `lib/shopping/service.ts`, dettagli in [07. Integrazioni](07-integrations.md)), che ne ricava nome, quantità e tipo di prodotto («x2 mele» → `mele`, `2`, `cibo`), e poi delega a `addItemAction` con urgenza `media`. Se il modello non è configurato, è lento o sbaglia, la riga entra così com'è scritta, senza quantità e in `cibo`: la barra rapida non può dipendere dall'IA. Quello che è stato letto torna al client per il toast, così chi aggiunge vede come è stato interpretato. Restituisce anche `duplicate` quando l'articolo è già in lista, così la barra rapida non rimette il testo nel campo.
