...
* `addQuickItemAction(text)`: Aggiunta dalla barra rapida della lista, dove si scrive una riga sola. Passa il testo a `readQuickItemInput` (→ `lib/shopping/service.ts`, dettagli in [07. Integrazioni](07-integrations.md)), che ne ricava nome, quantità e tipo di prodotto («x2 mele» → `mele`, `2`, `cibo`), e poi delega a `addItemAction` con urgenza `media`. Se il modello non è configurato, è lento o sbaglia, la riga entra così com'è scritta, senza quantità e in `cibo`: la barra rapida non può dipendere dall'IA. Quello che è stato letto torna al client per il toast, così chi aggiunge vede come è stato interpretato.
