...
* **Tastiera software**: `components/KeyboardInsets.tsx` (montato nel layout radice) misura con `visualViewport` e pubblica su `<html>` **due** variabili — calcolate da `measureKeyboard()` in `lib/keyboard.ts`, soglia 120px — più `data-keyboard="open"`:
