...
* `getOpenExpensesWithShares()` + `withContribution(open, userId)` → `lib/queries.ts`: la prima legge le spese aperte e le quote di **entrambi** da `v_expense_shares`, senza dipendere dall'utente, così parte nello stesso `Promise.all` di `getCurrentUser()` invece di aspettarlo; la seconda, pura, calcola il contributo netto individuale ($Anticipato - Quota$) in centesimi per l'utente corrente. Usate da home (anello, dettagli del saldo ed effetto sul saldo delle ultime spese) e conguaglio.
* `getOpenShares()` → `lib/queries.ts` + `contributionsById(expenses, shares, userId)` → `lib/spending.ts`: lo storico ha già tutte le spese da `getAllExpenses()`, quindi legge solo le quote delle aperte e ne ricava l'effetto sul saldo di chi guarda, per id. Stessa differenza in centesimi di `withContribution` (`expenseContribution` in `lib/spending.ts`, coperta da test); le saldate restano fuori, e una spesa senza quota (arrivata fra le due letture) resta senza effetto fino al prossimo aggiornamento invece di far fallire la pagina.
* `getExpenseById(id)` → `lib/queries.ts`: la spesa del dettaglio, con i join su chi l'ha pagata (`expenses_paid_by_fkey`), chi l'ha inserita (`expenses_created_by_fkey`) e, se saldata, il conguaglio che l'ha chiusa (`expenses_settlement_id_fkey`, con i nomi di chi ha versato e di chi ha ricevuto).
